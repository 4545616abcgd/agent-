...
static uint32_t diagnostic_hash(const char* value)
{
...
int volc_device_register(volc_iot_info_t* info, char** output)
{
    ...
    if (!signature || !root) {
        LOGE("Failed to allocate device registration request");
        ret = -1;
        goto err_out_label;
    }
    ...
    if (!json_str) {
        LOGE("Failed to serialize device registration request");
        ret = -1;
        goto err_out_label;
    }
    ...
    LOGD("registering hardware-agent device");
    ...
    LOGI("device registration accepted rtc_app_hash=%08" PRIx32,
         diagnostic_hash(info->rtc_app_id));

    // TODO: device secret, should be freed by caller
    *output = volc_aes_decode(info->product_secret, payload, true);
    if (!*output) {
        LOGE("Failed to decrypt registered device credentials");
        ret = -1;
    }
...
int volc_get_rtc_config(volc_iot_info_t* info, int audio_codec, const char* bot_id, const char* task_id, volc_room_info_t* room_info, const char* params) {
    ...
    cJSON* root = cJSON_CreateObject();
    if (!signature || !root) {
        LOGE("Failed to allocate RTC configuration request");
        ret = -1;
        goto err_out_label;
    }
    cJSON_AddStringToObject(root, "InstanceID", info->instance_id);
    ...
    json_str = cJSON_PrintUnformatted(root);
    if (!json_str) {
        LOGE("Failed to serialize RTC configuration request");
        ret = -1;
        goto err_out_label;
    }
    ...
    LOGI("requesting RTC configuration for registered device");
    response = volc_http_post(url, json_str, strlen(json_str));
    if (response == NULL) {
        LOGE("Failed to get response from server");
        ret = -1;
    ...
    response_json = cJSON_Parse(response);
    if (!response_json) {
        LOGE("Failed to parse RTC configuration response");
        ret = -1;
        goto err_out_label;
    }
    int server_error = 0;
    if (volc_json_read_int(response_json,
                           "ResponseMetadata.Error.CodeN",
                           &server_error) == 0) {
        ret = volc_inter_err_2_ext_err(server_error);
        LOGE("RTC configuration rejected code=%d mapped=%d (%s)",
             server_error, ret, volc_err_2_str(ret));
        goto err_out_label;
    }
    int room_ret = volc_json_read_string(response_json, "Result.RoomID", &room_info->rtc_opt.p_channel_name);
    int user_ret = volc_json_read_string(response_json, "Result.UserID", &room_info->rtc_opt.p_uid);
    int token_ret = volc_json_read_string(response_json, "Result.Token", &room_info->rtc_opt.p_token);
    int task_ret = volc_json_read_string(response_json, "Result.TaskID", &room_info->task_id);
    if (room_ret != 0 || user_ret != 0 || token_ret != 0 || task_ret != 0 ||
        room_info->rtc_opt.p_channel_name == NULL ||
        room_info->rtc_opt.p_uid == NULL ||
        room_info->rtc_opt.p_token == NULL || room_info->task_id == NULL) {
        LOGE("RTC configuration response is missing required fields");
        ret = -1;
        goto err_out_label;
    }
    LOGI("RTC configuration accepted task_id=%s room_hash=%08" PRIx32,
         room_info->task_id,
         diagnostic_hash(room_info->rtc_opt.p_channel_name));
err_out_label:
    if (root) {
        cJSON_Delete(root);
    }
