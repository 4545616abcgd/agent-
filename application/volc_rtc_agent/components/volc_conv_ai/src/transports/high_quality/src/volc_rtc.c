...
static int __rtc_start(rtc_impl_t* rtc, volc_rtc_option_t* option)
{
    ...
    char* channel_name = strdup(option->p_channel_name);
    char* user_id = strdup(option->p_uid);
    if (!channel_name || !user_id) {
        HAL_SAFE_FREE(channel_name);
        HAL_SAFE_FREE(user_id);
        LOGE("Failed to allocate RTC room identity");
        return -1;
    }
...
static void _on_user_joined(byte_rtc_engine_t engine, const char* channel, const char* user_name, int elapsed_ms)
{
    ...
    if (!rtc->p_remote_user_id) {
        rtc->b_user_joined = false;
        LOGE("Failed to allocate remote RTC user identity");
        return;
    }
...
static void _on_user_offline(byte_rtc_engine_t engine, const char* channel, const char* user_name, int reason)
{
    ...
    if (!rtc || !rtc->b_pipeline_started) {
        LOGD("ignoring late user-offline callback after local room leave");
        return;
    }
    rtc->b_user_joined = false;
    HAL_SAFE_FREE(rtc->p_remote_user_id);
...
{
    LOGI("RTC token privilege will expire");
    rtc_impl_t* rtc = (rtc_impl_t*) byte_rtc_get_user_data(engine);
    volc_msg_t msg_data = {0};
...
static int __rtc_init(rtc_impl_t* engine, cJSON* p_config)
{
            ...
            for (int i = 0; i < params_cnt; i++) {
                ...
                if (p_param && cJSON_IsString(p_param)) {
                    ...
                    if (param_str && strlen(param_str) > 0) {
                        ret = byte_rtc_set_params(engine->rtc, param_str);
                        if (ret != 0) {
                            LOGE("volc_rtc_create: param[%d] rejected: %d", i, ret);
    ...
    ret = byte_rtc_init(engine->rtc);
    if (ret != 0) {
        LOGE("volc_rtc_create: byte_rtc_init failed: %d", ret);
        return ret;
    }
    ret = byte_rtc_set_audio_codec(engine->rtc, __volc_to_rtc_audio_codec(engine->audio_codec));
    if (ret != 0) {
        LOGE("volc_rtc_create: audio codec setup failed: %d", ret);
        return ret;
    }
    ret = byte_rtc_set_video_codec(engine->rtc, video_codec - 1); // -1 for default codec
    if (ret != 0) {
        LOGE("volc_rtc_create: video codec setup failed: %d", ret);
        return ret;
    }
    return 0;
}
...
int volc_rtc_send(volc_rtc_t handle, const void* data, int size, volc_data_info_t* data_info) {
    ...
    switch (data_info->type) {
        case VOLC_DATA_TYPE_AUDIO: {
            ...
            ret = byte_rtc_send_audio_data(rtc->rtc, rtc->p_channel_name, data, size, &audio_info);
            break;
        }
        case VOLC_DATA_TYPE_VIDEO: {
            ...
            ret = byte_rtc_send_video_data(rtc->rtc, rtc->p_channel_name, data, size, &video_info);
            break;
        }
        case VOLC_DATA_TYPE_MESSAGE: {
            ...
            if (!rtc->p_remote_user_id) {
                LOGE("cannot send message before remote agent joins");
                return -1;
            }
            message_id = byte_rtc_rts_send_message(
                rtc->rtc, rtc->p_channel_name, rtc->p_remote_user_id,
