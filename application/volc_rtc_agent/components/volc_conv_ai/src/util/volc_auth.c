...
char* volc_aes_decode(const char* secret, const char* input, const bool partial_secret) {
    ...
    uint8_t *output = NULL;
    if (secret == NULL || input == NULL || strlen(input) == 0) {
        LOGE("Secret or encrypted input is empty");
        return NULL;
    }

    secret_len = strlen(secret);
    if (secret_len < (partial_secret ? AES_KEY_SIZE : AES_KEY_SIZE_24)) {
        LOGE("Secret length is too short for registration payload");
        return NULL;
    }
    ...
    if (partial_secret) {
        ...
        if (!key) {
            LOGE("Failed to allocate registration decryption key");
            return NULL;
        }
    ...
    LOGV("decoded registration payload bytes: %zu", decoded_len);

    output = volc_osal_calloc(decoded_len + 16, 1);
    if (!output) {
        LOGE("Failed to allocate decrypted registration payload");
    ...
    LOGV("decrypted registration payload bytes: %d",
